...
# INTERVAL seconds to os-usage.log — the ground-truth consumption numbers that pprof
...
#
# If --sched-container is given, also logs the kube-scheduler docker container's CPU%
# and resident memory every INTERVAL seconds to sched-usage.log (via `docker stats`),
# so a run captures the scheduler's consumption alongside minkapi's.
#
# Usage (all arguments are named flags; order does not matter):
#   ./profile.sh [--duration SECS] [--interval SECS] [--base-url URL] \
#                [--outdir DIR] [--pid PID] [--sched-container NAME] \
...
#   ./profiles/<timestamp>[-n<NODES>-p<PODS>-w<WORKERS>]
#   e.g. ./profiles/20260730-142530-n50000-p100000-w100
#   Field order is fixed: timestamp, then n<nodes>, p<pods>, w<workers>. Any of the
#   three --nodes/--pods/--workers that are supplied are appended in THAT order; ones
#   omitted are simply left out. An explicit --outdir overrides this entirely.
#
...
if [ -n "$PID" ] && kill -0 "$PID" 2>/dev/null; then
  echo "sampling OS CPU%/RSS of minkapi pid $PID -> $OUTDIR/os-usage.log"
else
...
# Columns (via `ps -o %cpu,rss` on the minkapi PID; the whole process, not just Go):
...
if [ -n "$SCHED_CONTAINER" ]; then
  if docker inspect "$SCHED_CONTAINER" >/dev/null 2>&1; then
    echo "sampling docker stats of scheduler container $SCHED_CONTAINER -> $OUTDIR/sched-usage.log"
    {
...
[ -n "$SCHED_PID" ] && wait "$SCHED_PID" 2>/dev/null || true
...
Inspect with:
  ...
  cat $OUTDIR/os-usage.log            # OS-level CPU% and RSS over the run
  cat $OUTDIR/mem-usage.log           # Go runtime heap/stack/Sys (MiB) over the run
  cat $OUTDIR/sched-usage.log         # kube-scheduler container CPU% and mem (if sampled)
EOF
